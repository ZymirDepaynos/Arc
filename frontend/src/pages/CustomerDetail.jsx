...
export default function CustomerDetail() {
  ...
  const [confirmDeleteHistoryIndex, setConfirmDeleteHistoryIndex] =
    useState(null);
  ...
  const handleEdit = async (form) => {
    ...
    if (parseFloat(old.original_debt || old.balance) !== rawBalance) {
      changes.push(
        `Total Amount of Purchased: ${fmt(old.original_debt || old.balance)} → ${fmt(rawBalance)}`,
      );
    }
    ...
    if (
      JSON.stringify([...oldReceipts].sort()) !==
      JSON.stringify([...newReceipts].sort())
    ) {
    ...
    await toast.promise(
      api.put(`/api/customers/${id}`, payload).then((r) => setCustomer(r.data)),
      {
        loading: "Saving...",
        success: "Saved!",
  ...
  const handleDeleteHistory = async (index) => {
    await toast.promise(api.delete(`/api/customers/${id}/history/${index}`), {
  ...
  const handleExport = () => {
    ...
    const receiptText = customer.receipt_numbers?.length
      ? customer.receipt_numbers.map((r) => `# ${r}`).join(", ")
      : "N/A (Pending)";
    ...
    doc.text("TRANSACTION REPORT", pageW / 2, 10 + logoH + 8, {
      align: "center",
    });
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      fmtPDF(customer.original_debt || customer.balance),
      margin + 55,
      mY1,
    );
    ...
    doc.setTextColor(40, 40, 40);
    doc.text(
      isSettled ? "P0.00 (Paid)" : fmtPDF(customer.balance),
      margin + 55,
      mY2,
    );
    ...
    if (customer.payment_history) {
      customer.payment_history.forEach((payment) => {
        ...
        const isDateOnly =
          payment.date &&
          (payment.date.length === 10 || !payment.date.includes("T"));
        ...
        events.push({
          ...
          dateStr: pDate.toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
            year: "numeric",
          }),
    ...
    events.sort((a, b) => {
      // Settled is always the last event chronologically (ascending sort)
      if (a.type === "settled" && b.type !== "settled") return 1;
      if (b.type === "settled" && a.type !== "settled") return -1;
    ...
    events.forEach((ev) => {
      if (ev.type === "created") {
        ...
        const amountStr =
          amtNum !== 0
            ? amtNum < 0
              ? `P + ${parseFloat(Math.abs(amtNum)).toLocaleString("en-PH", { minimumFractionDigits: 2 })}`
              : `P - ${parseFloat(amtNum).toLocaleString("en-PH", { minimumFractionDigits: 2 })}`
            : "";

        const balStr =
          p.balance_after !== undefined
            ? p.balance_after <= 0
              ? "P0.00 (Paid)"
              : fmtPDF(p.balance_after)
            : "";
    ...
    autoTable(doc, {
      ...
      didDrawPage: (hookData) => {
        ...
        doc.text(`Page ${currentPage} of ${pageCount}`, pageW / 2, pageH - 8, {
          align: "center",
        });
        doc.text(`Generated on ${generatedDate}`, pageW - margin, pageH - 8, {
          align: "right",
        });
      },
    });
        ...
        <div className="lg:col-span-1 flex flex-col gap-8">
          ...
          >
            ...
            >
              ...
              >
                {customer.receipt_numbers?.length
                  ? customer.receipt_numbers.map((r) => `#${r}`).join(", ")
                  : "—"}
              </div>
            </div>
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    fontWeight: 700,
                    background:
                      activeTab === "edits" ? "var(--bg-card)" : "transparent",
                    color:
                      activeTab === "edits"
                        ? "var(--text-primary)"
                        : "var(--text-muted)",
                    border: "none",
                    boxShadow:
                      activeTab === "edits" ? "var(--shadow-sm)" : "none",
                    cursor: "pointer",
                    transition: "all 0.2s",
                  }}
                >
              ...
              {(() => {
                ...
                if (activeTab === "transactions") {
                  events.push({
                    ...
                    dateStr: customer.date_borrowed
                      ? new Date(
                          customer.date_borrowed + "T12:00:00",
                        ).toLocaleDateString("en-US", {
                          month: "short",
                          day: "numeric",
                          year: "numeric",
                  ...
                  if (customer.payment_history) {
                    customer.payment_history.forEach((payment, idx) => {
                      if (
                        payment.type !== "edit" &&
                        payment.type !== "manual_adjustment"
                      ) {
                        const isDateOnly =
                          payment.date &&
                          (payment.date.length === 10 ||
                            !payment.date.includes("T"));
                        const pDate = isDateOnly
                          ? new Date(payment.date + "T12:00:00")
                          : new Date(payment.date);
                  ...
                  if (isSettled) {
                    events.push({
                      ...
                      type: "settled",
                      timestamp: new Date(customer.updated_at).getTime(),
                      dateStr: new Date(customer.updated_at).toLocaleDateString(
                        "en-US",
                        { month: "short", day: "numeric", year: "numeric" },
                      ),
                    });
                ...
                events.sort((a, b) => {
                  // Settled is always the newest event (descending sort)
                  if (a.type === "settled" && b.type !== "settled") return -1;
                  if (b.type === "settled" && a.type !== "settled") return 1;
                ...
                return events.map((ev) => {
                      ...
                      <div key={ev.id} className="timeline-item">
                        ...
                        <div className="timeline-content">
                          ...
                          <div className="timeline-desc">
                            Total amount of{" "}
                            <span className="timeline-money">
                              {fmt(
                                customer.original_debt ||
                                  customer.balance +
                    ...
                    return (
                      <div key={ev.id} className="timeline-item">
                        ...
                        <div className="timeline-content">
                          ...
                          <div className="timeline-desc">
                            Has been fully paid and closed.
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  return (
                    <div key={ev.id} className="timeline-item">
                      <div
                        className={`timeline-dot ${p.type === "edit" ? "status" : p.type === "manual_adjustment" ? "created" : "payment"}`}
                      ></div>
                      <div className="timeline-content">
                        <div className="timeline-time">{ev.dateStr}</div>
                        <div
                          ...
                          {p.type === "edit" ? (
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const raw =
                                    parseFloat(editHistoryAmount) || 0;
                                  if (raw === parseFloat(p.amount)) {
                                    toast.error(
                                      "No changes made to the payment amount",
                                    );
                                    return;
                                  }
                                  if (raw <= 0) {
