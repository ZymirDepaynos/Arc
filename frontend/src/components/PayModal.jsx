...
import {
  getToday,
  parseNaturalDate as parseNaturalDateUtil,
  formatDisplayDate,
...
export default function PayModal({ open, onClose, customer, onPay }) {
  ...
  const handlePay = async () => {
    ...
    if (payAmt > currentBalance) {
      toast.error(
        "Payment cannot exceed remaining balance (" + fmt(currentBalance) + ")",
      );
      return;
    }
    if (!date) {
    ...
    if (parsedState === "future_error") {
      return (
        <div style={{ ...baseStyle, color: "#ef4444" }}>
          Post-dated entries are not allowed
        </div>
      );
    }
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (val > getToday()) {
                      ...
                      customer.date_borrowed &&
                      val < customer.date_borrowed
                    ) {
                      setDateParsed("past_error");
                      setDate("");
                    } else {
